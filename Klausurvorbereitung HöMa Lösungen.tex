...
\begin{enumerate}
	...
	\item $\sum_{k=0}^\infty \frac{(-1)^k 2^{k-1} - 6 \cdot 5^k}{3^{2k+1}}$
		\begin{align*}
			...
			&= \frac{1}{6} \underbrace{\sum_{k=0}^\infty \left(\frac{-2}{9}\right)^k}_{geom. Reihe} - \underbrace{\sum_{k=0}^\infty \left(\frac{5}{9}\right)^k}_{geom. Reihe}
...
\begin{tabular}{c|ccccc} %TODO wie ist die f'-Zeile zu lesen?
	...
	$f(x)$ & $+\infty$ & $\backslash$ & $\approx -2,0646$ & / & $+\infty$\\
	$f'(x)$ & $-$ &  & 0 &  & $+$\\
\end{tabular}
\end{figure}\\
